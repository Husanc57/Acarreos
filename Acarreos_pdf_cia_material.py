...
class Imprime_Pdf_Cia_Material():
    ...
    def exportar_a_pdf(data):
        ...
        content = []
        title = Paragraph("REPORTE ACARREOS POR EMPRESA y MATERIAL", styles['Title'])
        ...
        for row in data[1:]:
            ...
            lst = list(row)
            lst[12] = r
            row = tuple(lst)#Como actualizar un valor en una lista dentro de un for loop
            print(r)
    ...
    def main_pdf_cia_material(contratista1, material1):
        ...
        with tab1:
            ...
            st.header("Datos de Acarreos")
            try:
                conn = sqlite3.connect("Acarreo_Proyectos.db")
                df = pd.read_sql_query(f"SELECT * FROM datos_acarreo WHERE contratista = '{contratista1}' AND material = '{material1}'", conn)
                st.dataframe(df)
        ...
        with tab2:
            ...
            if st.button("Generar Reporte PDF"):
                try:
                    ...
                    cursor.execute(f"SELECT * FROM datos_acarreo WHERE contratista = '{contratista1}' AND material = '{material1}'")
                    ...
                    pdf_buffer = Imprime_Pdf_Cia_Material.exportar_a_pdf(data)
                
                    st.download_button(
                        label="Descargar Reporte PDF",
                        data=pdf_buffer,
                        file_name="Reporte_Fecha.pdf",
                        mime="app")
...
if __name__ == "__main__":
    Imprime_Pdf_Cia_Material.main_pdf_cia_material()
