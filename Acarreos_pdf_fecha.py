...
class Imprime_Pdf_Fecha():
    ...
    def exportar_a_pdf(data):
        ...
        content = []
        title = Paragraph("REPORTE ACARREOS POR FECHA", styles['Title'])
    ...
    def main_pdf_fecha(fecha1):
        ...
        with tab1:
            ...
            try:
                ...
                df = pd.read_sql_query(f"SELECT * FROM datos_acarreo WHERE fecha = '{fecha1}'", conn)
                st.dataframe(df)
        ...
        with tab2:
            ...
            if st.button("Generar Reporte PDF"):
                try:
                    ...
                    cursor.execute(f"SELECT * FROM datos_acarreo WHERE fecha = '{fecha1}'")
                    ...
                    pdf_buffer = Imprime_Pdf_Fecha.exportar_a_pdf(data)
        ...
        with tab3:
            ...
            try:
                conn = sqlite3.connect("Acarreo_Proyectos.db")
                df = pd.read_sql_query(f"SELECT * FROM datos_acarreo WHERE datos_acarreo.fecha = '{fecha1}'", conn)
